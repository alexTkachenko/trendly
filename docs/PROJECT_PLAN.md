...
| Task | Description | Acceptance criteria | Agent | Status |
|---|---|---|---|---|
| DB-1.1 | Provision local Postgres via docker-compose (coordinate with E-OPS-1) | `docker compose up` starts a reachable Postgres with a named volume | database-agent | Done |
| DB-1.2 | Set up Flyway (or Liquibase) in the backend module | `V1__init.sql` applies cleanly on a fresh DB; migration runs on app startup | database-agent | Done |
| DB-1.3 | Define naming/typing conventions doc | `docs/db/conventions.md` covers PK strategy, timestamps, naming | database-agent | Done |

### E-DB-2 — Core domain schema
| Task | Description | Acceptance criteria | Agent | Status |
|---|---|---|---|---|
| DB-2.1 | `users` table (id, email, username, password_hash, display_name, bio, avatar_url, timestamps) — migration `V2`, also creates the shared `set_updated_at()` function and `docs/db/schema.md` | Unique constraints on email & username; migration + `docs/db/schema.md` entry | database-agent | Done |
| DB-2.2 | `posts` table (id, author_id FK, text_content, timestamps) — migration `V3` | FK to users with `ON DELETE CASCADE`; NOT NULL text or media required at app level | database-agent | Done |
| DB-2.3 | `post_media` table (id, post_id FK, url, media_type, position) | Supports multiple images per post; cascade delete with post | database-agent | Backlog |
| DB-2.4 | `follows` table (follower_id, followee_id, created_at) — migration `V4` | Composite primary key on (follower_id, followee_id) per `docs/db/conventions.md`, not a bare unique constraint; CHECK to prevent self-follow | database-agent | Done |
| DB-2.5 | `comments` table (id, post_id FK, author_id FK, content, timestamps) | Cascade delete with post; `author_id → users.id` is `ON DELETE CASCADE` per [ADR 002](adr/002-user-deletion-fk-behavior.md) (decided — `author_id` stays `NOT NULL`, no `SET NULL`) | database-agent | Done |
| DB-2.6 | `post_likes` table (post_id, user_id, created_at) | Composite primary key on (post_id, user_id) per `docs/db/conventions.md`, not a bare unique constraint (same mechanism as DB-2.4's `follows`); used for like counts | database-agent | Backlog |
...
| Task | Description | Acceptance criteria | Agent | Status |
|---|---|---|---|---|
| OPS-1.1 | `docker-compose.yml` with postgres, backend, frontend services + health checks | `docker compose up` brings the full stack up locally | devops-agent | Done |
| OPS-1.2 | Backend multi-stage Dockerfile | Image builds and runs; JAR built inside container | devops-agent | Done |
| OPS-1.3 | Frontend multi-stage Dockerfile | Image builds and serves the Next.js app | devops-agent | Done |
| OPS-1.4 | `.env.example` files for backend & frontend | Documented required vars; no secrets committed | devops-agent | Done |

### E-OPS-2 — CI pipeline
| Task | Description | Acceptance criteria | Agent | Status |
|---|---|---|---|---|
| OPS-2.1 | CI workflow: backend lint + unit + integration tests (`.github/workflows/backend-ci.yml`) | Pipeline fails on any test failure; runs against service-container Postgres | devops-agent | Done |
| OPS-2.2 | CI workflow: frontend lint + build + tests (`.github/workflows/frontend-ci.yml`) | Pipeline fails on lint/build/test errors | devops-agent | Done |
| OPS-2.3 | CI builds Docker images on merge to main | Images tagged with commit SHA | devops-agent | Backlog |
...
| Task | Description | Acceptance criteria | Agent | Status |
|---|---|---|---|---|
| BE-1.1 | Initialize Spring Boot project (web, data-jpa, security, validation, flyway) | Builds and starts; connects to Postgres from OPS-1.1 | backend-agent | Done |
| BE-1.2 | Global exception handling (`@ControllerAdvice`) + consistent error shape | Documented error response schema in `docs/api/errors.md` | backend-agent | Done |
| BE-1.3 | Base Spring Security config (stateless, JWT filter skeleton) per auth ADR | Unauthenticated requests to protected routes return 401 | backend-agent | Done |

### E-BE-2 — Auth & user management API
| Task | Description | Acceptance criteria | Agent | Status |
|---|---|---|---|---|
| BE-2.1 | `POST /auth/register` | Creates user, hashes password (BCrypt), validates uniqueness | backend-agent | Backlog |
| BE-2.2 | `POST /auth/login` issuing JWT | Valid credentials return access (+refresh) token; invalid return 401 | backend-agent | Backlog |
| BE-2.3 | `GET/PUT /users/me` (profile view/update) | Auth required; only owner can update; validation on fields | backend-agent | Backlog |
| BE-2.4 | `GET /users/{username}` public profile | Returns public fields only, 404 if not found | backend-agent | Backlog |
...
| Task | Description | Acceptance criteria | Agent | Status |
|---|---|---|---|---|
| FE-1.1 | Initialize Next.js (App Router, TypeScript, Tailwind) | Builds and runs; connects to backend base URL via env config | frontend-agent | Done |
| FE-1.2 | Typed API client scaffold against `docs/api/` contracts | Central fetch wrapper with auth header injection & error handling | frontend-agent | Backlog |
| FE-1.3 | App shell (layout, nav, auth-aware header) | Renders on all routes; shows login state correctly | frontend-agent | Backlog |
...
**Resolved:**
- ~~IDs: bigint identity vs UUID for primary keys?~~ → **RESOLVED: bigint identity**
  (`BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY`) uniformly on all tables, with no
  separate public UUID column. See [ADR 001](adr/001-primary-key-strategy.md).
  Unblocks DB-1.3 and E-DB-2 migrations.
- **Account deletion / FKs to `users`** (surfaced during DB-1.3, not in the original
  list above) → **RESOLVED: cascade everything.** Every FK referencing `users.id`
  (`posts.author_id`, `comments.author_id`, `follows.follower_id`/`followee_id`,
  `post_likes.user_id`) is `ON DELETE CASCADE`; deleting an account permanently
  removes all of that user's content, with no "[deleted user]" placeholder. See
  [ADR 002](adr/002-user-deletion-fk-behavior.md). Unblocks DB-1.3 re-review and
  DB-2.5; E-BE-2's account-deletion endpoint must document the destructive semantics.
